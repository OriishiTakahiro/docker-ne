...
# save YAML file
with open( "{dir}/{type}-{image}-{nodes}.yaml".format(dir=ARTIFACT_PATH, type=args.graph_type, image=args.image, nodes=args.nodes)  , "w") as f:
    f.write(dump( list(nodes.values()) ))

# save network graph
nx.draw_networkx(graph)
plt.savefig( "{dir}/{type}-{image}-{nodes}.svg".format(dir=ARTIFACT_PATH, type=args.graph_type, image=args.image, nodes=args.nodes), format="svg" )
